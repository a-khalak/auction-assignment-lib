...
public:
  ...
  inline matrix_market_stream(char* filename) {
    ...
    if (fin==0) {
      cerr << "Sorry, we can not open " << filename << endl;
      assert(0);
    }
    
    if (mm_read_banner(fin, &matcode) != 0) {
      cerr << "Sorry, we cannnot find the matrix market banner in " 
	   << filename << endl;
      fclose(fin);
      assert(0);
    }
    ...
    if (mm_is_coordinate(matcode) == 0 || mm_is_matrix(matcode) == 0) {
      cout << "file is not coordinate storage or is not a matrix" << endl;
      fclose(fin);
      assert(0);
    }

    if (mm_is_pattern(matcode)) {
      cout << "not currently supporting pattern" << endl;
      fclose(fin);
      assert(0);
    }
    
    if (mm_is_skew(matcode)) {
      cout << "not currently supporting skew symmetric" << endl;
      fclose(fin);
      assert(0);
    }
...
operator>>(matrix_market_stream<T>& mms, entry2<PR_>& e) {
  if ( mms.eof() ) { 
    ...
    if ( ! mms.is_complex() ) {  /* check the nuerical type */
      ...
      cout << "Numerical types of Matrix and entry are incompatible." << endl;
      assert(0);
    }
