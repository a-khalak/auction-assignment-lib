...
#ifdef MTL_BLAS_GROT
    T a = MTL_ABS(a_in), b = MTL_ABS(b_in);
...
inline void
__mult_generic(const Matrix& A, const VecX& xx, VecZ& zz) MTL_THROW_ASSERTION
{
...
template <class Matrix, class VecX, class VecZ>
inline void
__mult_shape(const Matrix& A, const VecX& x, VecZ& z,
             banded_tag) MTL_THROW_ASSERTION
{
  __mult_generic(A, x, z);
}
...
__mult_shape(const Matrix& A, const VecX& x, VecZ& z, 
             triangle_tag)
{
  __mult_shape(A, x, z, rectangle_tag());
  if (A.is_unit()) {
...
template <class Matrix, class VecX, class VecZ>
inline void
__mult_symm(const Matrix& A, const VecX& x, VecZ& z, row_tag)
{
  typedef typename matrix_traits<Matrix>::value_type T;
  typename Matrix::const_iterator i;
  typename Matrix::OneD::const_iterator j, jend;
...
template <class Matrix, class VecX, class VecZ>
inline void
__mult_symm(const Matrix& A, const VecX& x, VecZ& z, column_tag)
{
  typedef typename matrix_traits<Matrix>::value_type T;
  typename Matrix::const_iterator i;
  typename Matrix::OneD::const_iterator j, jend;
...
inline void
__mult_dim(const Matrix& A, const VecX& x, VecY& y, oned_tag) MTL_THROW_ASSERTION
{
  mult(A, x, mtl::scaled(y, 0), y);
#if 0
  typedef typename matrix_traits<Matrix>::shape Shape;
  __mult_shape(A, x, y, Shape());
#endif
}
...
template <class MatA, class MatB, class MatC>
inline void
__mult_dim(const MatA& A, const MatB& B, MatC& C, twod_tag)
{
...
{
  __tri_solve(T, const_cast<MatB&>(B), s);
}
...
template <class MatA, class MatB>
inline void
_twod_copy(const MatA& A, MatB& B, dense_tag)
{
...
template <class MatA, class MatB>
inline void
_twod_copy(const MatA& A, MatB& B, sparse_tag)
{
...
dot(const VecX& x, const VecY& y, T s, sparse_tag, sparse_tag)
{
  if (x.nnz() < y.nnz())
    ...
    return sparse_innder_product(y.begin(), y.end(), x.begin(), x.end(), s);
}
